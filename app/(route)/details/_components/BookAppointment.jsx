...
function BookAppointment({doctor}) {
  const { toast } = useToast()
    ...
    const saveBooking = ()=> {
      const data = {
          ...
          name : User.name ,
          email : User.email ,
          time : selectedTimeSlot , date : date ,
  ...
  return (
    <Dialog>
    <DialogTrigger>
    <Button className="mt-3 rounde
         d-full" >Book Appointment</Button>
    </DialogTrigger>
    <DialogContent>
      <DialogHeader>
                ...
                <div className="grid grid-cols-3 gap-2 border border-lg p-5 ">
                    {
                        timeSlot?.map((item , index)=> (
                            <h2 key={index} onClick={()=> setSelectedTimeSlot(item.time)}
                            className = {`p-2 border rounded-full hover:bg-primary text-center hover:text-white ${item.time == selectedTimeSlot && "bg-primary text-white " }`}  >{item.time}</h2>
                        ))
                    }
                </div>
                </div>
